...
def add_description(workbook, hike, days) -> None:
    ...
    for i, row in enumerate(dataclass):
        for j, item in enumerate(row):
            worksheet.write(i, j, item, get_text_format(workbook, bold=(j==0)))
...
def add_total_amount(workbook, statistic) -> None:
    ...
    worksheet.write(0, 0, 'Название продукта', get_text_format(workbook, bold=True))
    worksheet.write(0, 1, 'Общее количество', get_text_format(workbook, bold=True))
    worksheet.write(0, 2, '', get_text_format(workbook, bold=True))
    worksheet.write(0, 3, 'Количество приемов пищи', get_text_format(workbook, bold=True))
...
def add_border(workcheet, workbook, cols_count, rows_count):
    ...
    '''
    workcheet.merge_range(rows_count, 0, rows_count, cols_count - 1, '', get_text_format(workbook, border_top=True))
    workcheet.merge_range(0, cols_count, rows_count - 1, cols_count, '', get_text_format(workbook, border_left=True))
...
def add_menu(workbook, days) -> None:
    ...
    for i, day in enumerate(days):
        worksheet.merge_range(0, i * 5, 0, i * 5 + 4, f'{i + 1} день ({day.date})', get_text_format(workbook, bold=True, align='center', border=True))
        row = 1
        ...
        for eating in Eating.objects.filter(hikeDay=day).order_by('eating_category_id'):
            ...
            worksheet.merge_range(row, i * 5, row + max([10, len(ingredients)]) - 1, i * 5, eating.eating_category.name, get_text_format(workbook, bold=True, border=True, vertical=True))
            worksheet.merge_range(row, i * 5 + 1, row + max([10, len(ingredients)]) - 1, i * 5 + 1, eating.name, get_text_format(workbook, bold=True, border=True, vertical=True))
            set_width(worksheet, [i * 5 + j for j in range(5)], [3, 3, 18, 7, 3])

            for j, ingredient in enumerate(ingredients):
                amount =  get_amount_ingredient(ingredient)
                old_stat = statistic.get(ingredient.food.name, (0, '', 0))
                worksheet.write(row + j, i * 5 + 2, ingredient.food.name, get_text_format(workbook, border_top=j==0))
                worksheet.write(row + j, i * 5 + 3, amount, get_text_format(workbook, border_top=j==0))
                worksheet.write(row + j, i * 5 + 4, ingredient.food.unit, get_text_format(workbook, border_top=j==0))
                statistic[ingredient.food.name] = (old_stat[0] + amount, ingredient.food.unit, old_stat[2] + 1)
                
            row += max([10, len(ingredients)])

    add_total_amount(workbook, statistic)
    add_border(worksheet, workbook, len(days) * 5, row)
